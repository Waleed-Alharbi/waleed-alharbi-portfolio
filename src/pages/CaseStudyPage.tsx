...
export function CaseStudyPage() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    if (project) document.title = `${project.title[language]} — Waleed Alharbi`;
    return () => { document.title = 'Waleed Alharbi — Information Technology'; };
  }, [project, language]);
  ...
  return (
    <main className={`case-study case-${project.theme}`} id="main">
      <section className="case-hero">
        ...
        <div className="case-hero-bottom"><p>{project.summary[language]}</p><a href={project.github} target="_blank" rel="noreferrer">{t.openGithub}<span aria-hidden="true">{language === 'ar' ? '↖' : '↗'}</span></a></div>
